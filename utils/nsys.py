...
DOCKYARD_NSYS_WORKER_PATTERNS = os.environ.get("DOCKYARD_NSYS_WORKER_PATTERNS", "")
DOCKYARD_NSYS_PROFILE_STEP_RANGE = os.environ.get("DOCKYARD_NSYS_PROFILE_STEP_RANGE", "")
...
def maybe_gpu_profile_step(policy: ProfilablePolicy, step: int):
    assert not (
        bool(DOCKYARD_NSYS_WORKER_PATTERNS) ^ bool(DOCKYARD_NSYS_PROFILE_STEP_RANGE)
    ), (
        "Either both DOCKYARD_NSYS_WORKER_PATTERNS and DOCKYARD_NSYS_PROFILE_STEP_RANGE must be set, "
        "or neither. See docs/nsys-profiling.md for more details."
    )

    if not DOCKYARD_NSYS_WORKER_PATTERNS:
        return

    NSYS_START_STEP, NSYS_STOP_STEP = DOCKYARD_NSYS_PROFILE_STEP_RANGE.split(":")
    try:
        NSYS_START_STEP = int(NSYS_START_STEP)
        NSYS_STOP_STEP = int(NSYS_STOP_STEP)
    except ValueError as e:
        raise ValueError(
            f"Error parsing DOCKYARD_NSYS_PROFILE_STEP_RANGE: {str(e)}. "
            "Please ensure the format is 'start:stop' where both values are integers. "
            "See docs/nsys-profiling.md for more details."
        ) from e

    assert NSYS_START_STEP < NSYS_STOP_STEP, (
        f"{DOCKYARD_NSYS_PROFILE_STEP_RANGE=} must be a non-empty range"
    )
    assert NSYS_START_STEP >= 1, (
        f"The start step in {DOCKYARD_NSYS_PROFILE_STEP_RANGE=} must be >= 1"
    )

    # Use slice syntax of left inclusive and right exclusive
    if NSYS_START_STEP <= step < NSYS_STOP_STEP:
        if not getattr(policy, "__DOCKYARD_PROFILE_STARTED", False):
            rich.print(
                f"[bold red]Starting GPU profiling for {policy} for step {step}[/bold red]"
            )
            policy.start_gpu_profiling()
            policy.__DOCKYARD_PROFILE_STARTED = True
        ...
        if getattr(policy, "__DOCKYARD_PROFILE_STARTED", False):
            rich.print(
                f"[bold red]Stopping GPU profiling for {policy} for step {step}[/bold red]"
            )
            policy.stop_gpu_profiling()
            policy.__DOCKYARD_PROFILE_STARTED = False
