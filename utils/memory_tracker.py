...
try:
    from ray.scripts.scripts import memory_summary as _ray_memory_summary  # type: ignore[attr-defined]

    def _get_ray_memory_summary() -> str:
        return _ray_memory_summary(stats_only=True, num_entries=5)

except ImportError:
    def _get_ray_memory_summary() -> str:
        return "(ray.scripts.scripts.memory_summary unavailable)"
...
class MemoryTrackerDataPoint(BaseModel):
    ...
    def mem_used_diff_gb(self) -> float:
        assert self.memory_used_after_stage_gb is not None, (
            "memory_used_after_stage_gb is not set; call snapshot_start_of_stage first"
        )
        return self.memory_used_after_stage_gb - self.memory_used_before_stage_gb

    @property
    def new_variables(self) -> List[str]:
        assert self.variables_after_stage is not None, (
            "variables_after_stage is not set; call snapshot_start_of_stage first"
        )
        return [
            v
    ...
    def get_snapshot_str(self) -> str:
        ray_memory_summary = _get_ray_memory_summary()
