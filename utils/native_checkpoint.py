...
def save_checkpoint(
    ...
    dcp.save(model_state, checkpoint_id=weights_path)  # type: ignore[attr-defined]
    ...
    if optimizer is not None:
        ...
        dcp.save(optimizer_state, checkpoint_id=optimizer_path)  # type: ignore[attr-defined]
...
def load_checkpoint(
    ...
    dcp.load(state_dict=model_state_dict, checkpoint_id=weights_path)  # type: ignore[attr-defined]
    ...
    if optimizer is not None:
        ...
        dcp.load(state_dict=optimizer_state_dict, checkpoint_id=optimizer_path)  # type: ignore[attr-defined]
...
def convert_dcp_to_hf(
    ...
    This is not an optimized utility. If the checkpoint is too large, consider saving DCP
    during training and using this utility to convert to HF format offline.
    ...
    Args:
        ...
        hf_overrides (dict, optional): Additional kwargs forwarded to AutoConfig.from_pretrained.
    ...
    config = AutoConfig.from_pretrained(
        model_name_or_path, trust_remote_code=True, **(hf_overrides or {})
    )
    config.save_pretrained(hf_ckpt_path)

    # Tokenizer is loaded from tokenizer_name_or_path rather than from the checkpoint
    # directory, which provides a stable source regardless of training-loop changes.
