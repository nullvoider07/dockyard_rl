# Windows CUA backend (Phase 3, Thrust B). Drives a Windows guest (qcow2 in a
# KVM-in-container, e.g. Environments/windows{10,11}-base) where control-center,
# The-Eye, and the AHK command-file watchers are baked into the guest image:
#   observe  -> The-Eye  GET /snapshot.png
#   act      -> control-center ExecuteCommand :50051, via pyautogui -> Windows
#               cmd-echo to C:\mouse_cmd.txt / keyboard_cmd.txt (cc_transpile)
#   provision-> the same warm-pool provisioner as the Linux backend, with the
#               Windows container image / host pool (qcow2-overlay reset).
#
# Grading is intentionally NOT implemented: the OSWorld task suite is
# Ubuntu-authored (its evaluators target LibreOffice/GIMP/Chrome on Linux and run
# against the :5000 guest server, which the Windows guest does not run). Windows
# is infrastructure for *custom* task sets; their evaluators are Phase 4. evaluate
# therefore raises unless a custom grader is supplied via cfg['grader'].
...
from dockyard_rl.experience.cua.actions.cc_transpile import pyautogui_to_windows
...
logger = logging.getLogger("dockyard_rl.cua.backends.windows")
...
class WindowsBackend(OSWorldBackend):
    """OSWorldBackend for a Windows guest over control-center + The-Eye.

    Config keys (env.osworld; mirrors the control_center backend minus the Linux
    guest-server/grading bits):
        ...
        grader:  custom-task grading. A callable(task_config, handle) -> float, or
                 any truthy value (e.g. "custom") to use the registry-backed
                 dispatch in experience/cua/grading (with evaluator_modules /
                 judge); absent → evaluate raises (no grading configured).
    """
    ...
    def start(self, task: dict[str, Any]) -> _WindowsHandle:
    ...
    def observe(self, handle: _WindowsHandle) -> Observation:
        try:
        ...
        return Observation(
            ...
            accessibility_tree=None,  # Windows a11y is not wired (screenshot-primary)
            terminal=None,
            instruction=handle.task_config.get("instruction", ""),
        )

    def act(self, handle: _WindowsHandle, action: ParsedAction) -> None:
        ...
        if action.control == "fail":
            ...
            return
        if action.code:
            for cmd in pyautogui_to_windows(
                action.code,
    ...
    def evaluate(self, handle: _WindowsHandle) -> float:
        if self._grader is not None:
            return float(self._grader(handle.task_config, handle))
        raise NotImplementedError(
            "WindowsBackend has no grading configured: OSWorld's evaluators are "
            "Ubuntu-authored and run against the :5000 guest server, absent on the "
            "Windows guest. Set env.osworld.grader (e.g. 'custom') and author "
            "evaluators in experience/cua/grading for Windows task sets."
        )

    def reset(self, handle: _WindowsHandle, task: dict[str, Any]) -> Observation:
        handle.task_config = task
        handle.action_history.clear()
        return self.observe(handle)

    def stop(self, handle: _WindowsHandle) -> None:
        try:
            handle.cc.close()
