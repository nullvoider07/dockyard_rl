...
import zipfile
#import pandas as pd
from typing import Any, TypeVar, Union, Iterable, Mapping, Optional, Callable, cast
...
_xlsx_ns_imapping: Dict[str, Optional[str]] = dict(map(lambda itm: (itm[1], itm[0]), _xlsx_namespaces))
...
def load_sparklines(xlsx_file: str, sheet_name: str) -> Dict[str, str]:
    ...
    for sp_l in sparklines:
        ...
        sparkline: Dict[str, Dict[str, str]] = xmltodict.parse(sparkline_xml
                                                               ...
                                                               , namespaces=cast(Dict[str, str], _xlsx_ns_imapping)
                                                               )
...
def load_charts(xlsx_file: Workbook, sheet_name: str, **options) -> Dict[str, Any]:
    ...
    try:
        ...
        return {}
    charts: List[ChartBase] = getattr(worksheet, "_charts")
    ...
    for ch in charts:
        ...
        if "title" in chart_props:
            try:
                info["title"] = ch.title.tx.rich.p[0].r[0].t  # type: ignore[union-attr]
            except:
                info["title"] = None
        if "legend" in chart_props:
            info["legend"] = ch.legend.position if ch.legend is not None else None
        if "anchor" in chart_props:
            anchor = cast(Any, ch.anchor)
            info["anchor"] = [anchor.editAs
                , anchor._from.col, anchor.to.row
                , anchor.to.col, anchor.to.row
                              ]
        ...
        if "direction" in chart_props:
            info["direction"] = cast(Any, ch).barDir

        if "xtitle" in chart_props:
            try:
                info["xtitle"] = cast(Any, ch).x_axis.title.tx.rich.p[0].r[0].t
            except:
                info["xtitle"] = None
        if "ytitle" in chart_props:
            try:
                info["ytitle"] = cast(Any, ch).y_axis.title.tx.rich.p[0].r[0].t
            except:
                info["ytitle"] = None
        if "ztitle" in chart_props:
            try:
                info["ztitle"] = cast(Any, ch).z_axis.title.tx.rich.p[0].r[0].t
            except:
                info["ztitle"] = None
        chart_set[str(series_repr)] = info
...
def load_pivot_tables(xlsx_file: Workbook, sheet_name: str, **options) -> Dict[str, Any]:
    ...
    try:
        ...
        return {}
    pivots: List[PivotTableDefinition] = getattr(worksheet, "_pivots")
    ...
    for pvt in pivots:
        ...
        cell_range1_str, cell_range2_str = cache_source.worksheetSource.ref.split(":")  # type: ignore[union-attr]
        cell_range1 = coordinate_to_tuple(cell_range1_str)
        cell_range1 = (cell_range1[0], cell_range1[1] + left_bias)
        cell_range2 = coordinate_to_tuple(cell_range2_str)
...
def read_cell_value(xlsx_file: str, sheet_name: str, coordinate: str) -> Any:
    ...
    cell_dict: Dict[str, Any] = xmltodict.parse(lxml.etree.tostring(cell_el, encoding="unicode")
                                           , process_namespaces=True
                                           , namespaces=cast(Dict[str, str], _xlsx_ns_imapping)
                                           )
    ...
    try:
        if "@t" not in cell_dict["c"] or cell_dict["c"]["@t"] == "n":
            return float(cell_dict["c"]["v"])
        if cell_dict["c"]["@t"] == "s":
            return shared_strs[int(cell_dict["c"]["v"])]
        if cell_dict["c"]["@t"] == "str":
            return cell_dict["c"]["v"]
        if cell_dict["c"]["@t"] == "inlineStr":
            return cell_dict["c"]["is"]["t"]
        if cell_dict["c"]["@t"] == "e":
            return cell_dict["c"]["v"]
    ...
    for fmt in conditional_formattings:
        for r in fmt.rules:
            ...
            for rge in fmt.cells:
                for c in rge.cells:
                    cell: Cell = cast(Cell, worksheet.cell(row=c[0], column=c[1]))
...
def load_rows_or_cols(xlsx_file: Workbook, sheet_name: str, **options) \
    ...
    obj_set: Dict[Union[int, str], Any] = {}
...
def _match_value_to_rule(value: V, rule: Mapping[str, Union[str, V]]) -> bool:
    """
    Args:
    ...
    method: str = cast(str, rule["method"])
    if method.startswith("re"): # re.FLAGs
        flag_strs: List[str] = method.split(".")[1:]
        flags: Iterable[re.RegexFlag] = (getattr(re, fl) for fl in flag_strs)
        flag: re.RegexFlag = functools.reduce(operator.or_, flags, re.RegexFlag(0))
        logger.debug("REFLAG: %s", repr(flag))

        match_: Optional[Match[str]] = re.search(cast(str, rule["ref"]), cast(str, value), flag)
        return match_ is not None
    if method in {"eq", "ne"
        , "le", "lt"
        , "ge", "gt"
                          }:
        return getattr(operator, method)(value, rule["ref"])
    if method.startswith("approx"): # approx:THRESHOLD
        ...
        try:
            value_f: float = float(cast(Any, value))
        except (ValueError, TypeError):
            return False
        else:
            return abs(value_f - cast(Any, rule["ref"])) <= threshold
    if method == "spreadsheet_range":
        _rule_ref = cast(Any, rule["ref"])
        subset_limit = MultiCellRange(_rule_ref[0])
        superset_limit = MultiCellRange(_rule_ref[1])
        return _multicellrange_containsby(subset_limit, cast(MultiCellRange, value)) \
            and _multicellrange_containsby(cast(MultiCellRange, value), superset_limit)
    if method.startswith("range."):
        _rule_ref = cast(Any, rule["ref"])
        left_et = method[6]
        right_et = method[7]
        return getattr(operator, "l" + left_et)(_rule_ref[0], value) \
            and getattr(operator, "l" + right_et)(value, _rule_ref[1])
    if rule["method"] in {"str_list_eq", "str_set_eq"}:
        container_type_str: str = cast(str, rule["method"])[4:-3]
        container_type = getattr(builtins, container_type_str)

        value = container_type(cast(str, value).strip("\"'").split(","))
