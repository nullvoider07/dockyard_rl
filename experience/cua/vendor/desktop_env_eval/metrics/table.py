...
def _parse_sheet_idx(
    ...
    expected_sheet_names: Optional[List[str]],
) -> Tuple[BOOK, str]:
    ...
    if isinstance(sheet_idx, int):
        ...
        try:
            index: str = expected_sheet_names[int(sheet_idx[2:])] if expected_sheet_names else ""
        except:
            index = ""
        book: BOOK = expected
...
def compare_table(result: str, expected: Optional[str] = None, **options) -> float:
    ...
    worksheete_names: Optional[List[str]] = None
    ...
    for r in options["rules"]:
        if r["type"] == "sheet_name":
            ...
            if pdworkbooke is None:
                logger.error("Expected workbook is None for sheet_data comparison")
                return 0.0
            data_sheet1 = cast(Optional[pd.DataFrame], _load_sheet(
                *parse_idx(r["sheet_idx0"], pdworkbookr, pdworkbooke)
            ))
            if data_sheet1 is None:
                return 0.0
            data_sheet2 = cast(Optional[pd.DataFrame], _load_sheet(
                *parse_idx(r["sheet_idx1"], pdworkbookr, pdworkbooke)
            ))
            if data_sheet2 is None:
                return 0.0

            rounded1 = data_sheet1.round(error_limit)
            rounded2 = data_sheet2.round(error_limit)
            metric: bool = rounded1.equals(rounded2)
            logger.debug("Sheet1: \n%s", str(rounded1))
            logger.debug("Sheet2: \n%s", str(rounded2))
            try:
                logger.debug("Sheet1 =v= Sheet2: \n%s", str(rounded1 == rounded2))
            except:
                logger.debug("Sheet1 =/v= Sheet2")
            logger.debug(
            ...
            print_sheet1 = cast(Optional[List[str]], _load_sheet(
                *parse_idx(r["sheet_idx0"], result, cast(str, expected))
            ))
            if print_sheet1 is None:
                return 0.0
            print_sheet2 = cast(Optional[List[str]], _load_sheet(
                *parse_idx(r["sheet_idx1"], result, cast(str, expected))
            ))
            if print_sheet2 is None:
                return 0.0
            if r.get("ignore_case", False):
                print_sheet1 = [l.lower() for l in print_sheet1]
                print_sheet2 = [l.lower() for l in print_sheet2]
            metric: bool = print_sheet1 == print_sheet2
            logger.debug(
            ...
            fuzzy_sheet1: Tuple[BOOK, str] = parse_idx(r["sheet_idx0"], result, cast(str, expected))
            fuzzy_sheet2: Tuple[BOOK, str] = parse_idx(r["sheet_idx1"], result, cast(str, expected))
            total_metric = True
            for rl in r["rules"]:
                for rng in MultiCellRange(rl["range"]):
                    for cdn in rng.cells:
                        ...
                        value1: str = str(read_cell_value(cast(str, fuzzy_sheet1[0]), fuzzy_sheet1[1], coordinate))
                        value2: str = str(read_cell_value(cast(str, fuzzy_sheet2[0]), fuzzy_sheet2[1], coordinate))
                        ...
                        if "ignore_chars" in rl:
                            ...
                            value1 = "".join(
                                ch for ch in value1 if ch not in ignore_chars
                            )
                            value2 = "".join(
                                ch for ch in value2 if ch not in ignore_chars
                            )
                        ...
                        if rl["type"] == "includes":
                            ...
                            raise NotImplementedError(
                                "Unknown sheet_fuzzy rule type: {:}".format(rl["type"])
                            )
            ...
            if expected is None:
                logger.error("Expected file is None for sparkline comparison")
                return 0.0
            _idx0 = parse_idx(r["sheet_idx0"], result, expected)
            sparkline1: Dict[str, str] = load_sparklines(
                cast(str, _idx0[0]), _idx0[1]
            )
            _idx1 = parse_idx(r["sheet_idx1"], result, expected)
            sparkline2: Dict[str, str] = load_sparklines(
                cast(str, _idx1[0]), _idx1[1]
            )
            ...
            if xlworkbooke is None:
                logger.error("Expected workbook is None for chart comparison")
                return 0.0
            _cidx0 = parse_idx(r["sheet_idx0"], xlworkbookr, xlworkbooke)
            charts1: Dict[str, Any] = load_charts(cast(Workbook, _cidx0[0]), _cidx0[1], **r)
            _cidx1 = parse_idx(r["sheet_idx1"], xlworkbookr, xlworkbooke)
            charts2: Dict[str, Any] = load_charts(cast(Workbook, _cidx1[0]), _cidx1[1], **r)
            ...
            if xlworkbooke is None or expected is None:
                logger.error("Expected workbook is None for style comparison")
                return 0.0
            sheet_idx1 = parse_idx(r["sheet_idx0"], xlworkbookr, xlworkbooke)
            book_name1: str = cast(str, parse_idx(r["sheet_idx0"], result, expected)[0])
            styles1: Dict[str, List[Any]] = load_xlsx_styles(
                cast(Workbook, sheet_idx1[0]), sheet_idx1[1], book_name1, **r
            )

            sheet_idx2 = parse_idx(r["sheet_idx1"], xlworkbookr, xlworkbooke)
            book_name2: str = cast(str, parse_idx(r["sheet_idx1"], result, expected)[0])
            styles2: Dict[str, List[Any]] = load_xlsx_styles(
                cast(Workbook, sheet_idx2[0]), sheet_idx2[1], book_name2, **r
            )
            ...
            sheet1 = cast(Optional[Worksheet], _load_sheet(
                *parse_idx(r["sheet_idx0"], xlworkbookr, cast(Workbook, xlworkbooke))
            ))
            if sheet1 is None:
                return 0.0
            sheet2 = cast(Optional[Worksheet], _load_sheet(
                *parse_idx(r["sheet_idx1"], xlworkbookr, cast(Workbook, xlworkbooke))
            ))
            if sheet2 is None:
                return 0.0
            ...
            sheet = cast(Optional[Worksheet], _load_sheet(
                *parse_idx(r["sheet_idx"], xlworkbookr, cast(Workbook, xlworkbooke))
            ))
            if sheet is None:
                return 0.0
            zoom_scale: float = sheet.sheet_view.zoomScale or 100.0
            ...
            if xlworkbooke is None:
                logger.error("Expected workbook is None for row_props comparison")
                return 0.0
            _ridx0 = parse_idx(r["sheet_idx0"], xlworkbookr, xlworkbooke)
            rows1: Dict[Union[int, str], Dict[str, Any]] = load_rows_or_cols(cast(Workbook, _ridx0[0]), _ridx0[1], obj="row", **r)
            _ridx1 = parse_idx(r["sheet_idx1"], xlworkbookr, xlworkbooke)
            rows2: Dict[Union[int, str], Dict[str, Any]] = load_rows_or_cols(cast(Workbook, _ridx1[0]), _ridx1[1], obj="row", **r)
            ...
            if xlworkbooke is None:
                logger.error("Expected workbook is None for col_props comparison")
                return 0.0
            _coidx0 = parse_idx(r["sheet_idx0"], xlworkbookr, xlworkbooke)
            cols1: Dict[Union[int, str], Dict[str, Any]] = load_rows_or_cols(cast(Workbook, _coidx0[0]), _coidx0[1], obj="column", **r)
            _coidx1 = parse_idx(r["sheet_idx1"], xlworkbookr, xlworkbooke)
            cols2: Dict[Union[int, str], Dict[str, Any]] = load_rows_or_cols(cast(Workbook, _coidx1[0]), _coidx1[1], obj="column", **r)
            ...
            if xlworkbooke is None:
                logger.error("Expected workbook is None for filter comparison")
                return 0.0
            _fidx0 = parse_idx(r["sheet_idx0"], xlworkbookr, xlworkbooke)
            filters1: Dict[str, Any] = load_filters(cast(Workbook, _fidx0[0]), _fidx0[1], **r)
            _fidx1 = parse_idx(r["sheet_idx1"], xlworkbookr, xlworkbooke)
            filters2: Dict[str, Any] = load_filters(cast(Workbook, _fidx1[0]), _fidx1[1], **r)
            ...
            if xlworkbooke is None:
                logger.error("Expected workbook is None for pivot_table comparison")
                return 0.0
            _pidx0 = parse_idx(r["sheet_idx0"], xlworkbookr, xlworkbooke)
            pivots1: Dict[str, Any] = load_pivot_tables(cast(Workbook, _pidx0[0]), _pidx0[1], **r)
            _pidx1 = parse_idx(r["sheet_idx1"], xlworkbookr, xlworkbooke)
            pivots2: Dict[str, Any] = load_pivot_tables(cast(Workbook, _pidx1[0]), _pidx1[1], **r)
            ...
            try:
                ...
                cell: Cell = cast(Cell, sheet[r["coordinate"]])
                metric: bool = True
                for prpt, rule in r["props"].items():
                    if prpt == "value":
                        try:
                            if expected is None:
                                logger.error("Expected file is None for check_cell value read")
                                val = None
                            else:
                                parsed_result = parse_idx(r["sheet_idx"], result, expected)
                                logger.debug(f"parse_idx result: {parsed_result}")
                                val = read_cell_value(cast(str, parsed_result[0]), parsed_result[1], r["coordinate"])
...
def compare_conference_city_in_order(actual_city_list_path, expected_city):
    ...
    sheet = wb.active
    if sheet is None:
        logger.error("No active sheet in %s", actual_city_list_path)
        return 0.0
    actual_city_list = []
