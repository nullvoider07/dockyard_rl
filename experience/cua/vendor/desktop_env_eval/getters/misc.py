...
def get_rule(env, config: Dict[str, Rule]) -> Rule:
    """
...
def get_rule_relativeTime(env, config: Dict[str, Any]) -> Any:
    """
    ...
    relativeRules: Dict[str, Any] = config["rules"]
    relativeTime: Dict[str, Any] = relativeRules["relativeTime"]
    ...
    if "to" not in relativeTime.keys():
        ...
        absoluteDay: Optional[datetime] = None
        ...
        from_absoluteDay: Optional[datetime] = None
        ...
        if from_time == "next Monday split":
            if from_absoluteDay is not None:
        ...
        # deal with to_time
        to_absoluteDay: Optional[datetime] = None
...
def apply_rules_to_timeFormat(timeFormat: str, absoluteDay: datetime) -> str:
...
def get_time_diff_range(env, config) -> Optional[str]:
    try:
...
def get_timezone_from_config(config: Dict, default_timezone: Optional[str] = None) -> str:
    """
