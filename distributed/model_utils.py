...
try:
    from dockyard_rl.algorithms.logits_sampling_utils import (
        TrainingSamplingParams as TrainingSamplingParams,
        apply_top_k_top_p as apply_top_k_top_p,
        need_top_k_or_top_p_filtering as need_top_k_or_top_p_filtering,
    )
...
def dtensor_from_parallel_logits_to_logprobs(
    ...
    if cp_size > 1:
        ...
        assert cp_mesh is not None and cp_placements is not None and sorted_indices is not None and target_shape is not None
...
class AllGatherCPTensor(torch.autograd.Function):
    @staticmethod
    def forward(  # pyrefly: ignore[bad-override]
        ctx: Any, tensor: torch.Tensor, cp_group: torch.distributed.ProcessGroup, seq_dim: int = 1
    ) -> torch.Tensor:  # , unpadded_seqlen: Optional[int] = None):
...
def get_next_token_logprobs_from_logits(
    ...
    1. Vocab parallel: uses from_parallel_logits_to_logprobs
    ...
    if vocab_parallel_group is not None:
        ...
        dtensor_logits: DTensor = next_token_logits
