...
def parse_conversations(tree_obj, first: bool = False):
    """Recursive function that returns all the sub converstaiions in a list starting from node tree_obj.
...
class OasstDataset(RawDataset):
    ...
    def __init__(self, split_validation_size: float = 0.05, seed: int = 42, **kwargs):
        ...
        records = get_data_records(all_objs, task_name=self.task_name)
