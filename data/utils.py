...
from dockyard_rl.data.datasets import (
    AllTaskProcessedDataset,
    extract_necessary_env_names,
    load_response_dataset,
    update_single_dataset_config,
)
from dockyard_rl.data.processors import preference_preprocessor
...
def setup_response_data(
    ...
    This function is used to setup the data and environments for the training
    ...
    Args:
        ...
        env_configs: Environment configs.
            ...
            - Environments which need to handle the environment creation outside
              of this function (e.g. custom sandbox environments).
        is_vlm: Whether to use VLM training or not.

    Returns:
        If env_configs is not None:
            A tuple of (train dataset, validation dataset, task to environment,
            task to validation environment).
        If env_configs is None:
            A tuple of (train dataset, validation dataset).
    """
    assert "train" in data_config, (
        "The dataset config structure is updated. Please refer to the Dockyard "
        "documentation and update the dataset config."
    )
    ...
    if has_envs:
        print("\n▶ Setting up envs...")
        env_name_list = extract_necessary_env_names(cast(dict, data_config))
        envs = {}
        for env_name in env_name_list:
            # Deferred import to avoid circular dependency with environments layer
            from dockyard_rl.environments.utils import create_env  # type: ignore[import]
    ...
    if isinstance(data_config["train"], dict):
        data_config["train"] = [data_config["train"]]  # type: ignore[typeddict-item]

    for cfg in data_config["train"]:
        # load dataset
        if "default" in data_config and data_config["default"] is not None:
            update_single_dataset_config(cast(dict, cfg), cast(dict, data_config["default"]))
        data = load_response_dataset(cast(Any, cfg))
        ...
        if has_envs:
            task_to_env[task_name] = envs[cfg["env_name"]]  # type: ignore[typeddict-item]
    ...
    if "validation" in data_config and data_config["validation"] is not None:
        if isinstance(data_config["validation"], dict):
            data_config["validation"] = [data_config["validation"]]  # type: ignore[typeddict-item]

        for cfg in data_config["validation"]:
            # load dataset
            if "default" in data_config and data_config["default"] is not None:
                update_single_dataset_config(cast(dict, cfg), cast(dict, data_config["default"]))
            val_data = load_response_dataset(cast(Any, cfg))
            ...
            if has_envs:
                val_task_to_env[task_name] = envs[cfg["env_name"]]  # type: ignore[typeddict-item]
    ...
    # Structured tool-use protocol (grpo.structured_tool_use): attach the block
    # to every task spec so tool-use processors can advertise the env's tool
    # registry in the prompt. None → inert, fenced-text prompts unchanged.
    if structured_tool_use is not None:
        for _spec, _ in task_data_processors.values():
            _spec.structured_tool_use = structured_tool_use
        for _spec, _ in val_task_data_processors.values():
            _spec.structured_tool_use = structured_tool_use
...
def setup_preference_data(
    ...
    This function is used to setup the preference data for the training and
    validation datasets.
    ...
    from dockyard_rl.data.datasets.preference_datasets import load_preference_dataset  # type: ignore[import]

    print("\n▶ Setting up data...")
    # setup train dataset
    if "default" in data_config:
        update_single_dataset_config(cast(dict, data_config["train"]), cast(dict, data_config["default"]))
    data = load_preference_dataset(cast(Any, data_config["train"]))
    ...
    if "val_data_paths" in data_config and data_config["val_data_paths"]:
        assert isinstance(data_config["val_data_paths"], dict), (
            ...
            "val_data_paths must be a dictionary."
        )
        ...
        for val_dataset_name, val_dataset_path in val_data_paths.items():
            ...
            val_dataset[val_dataset_name] = AllTaskProcessedDataset(
                ...
                cast(Any, tokenizer),
                cast(Any, None),
                cast(Any, val_task_data_processors),
                task_data_preprocessors=val_task_data_preprocessors,
                max_seq_length=data_config["max_input_seq_length"],
            )
            print(
        ...
        if "default" in data_config:
            update_single_dataset_config(
                cast(dict, data_config["validation"]), cast(dict, data_config["default"])
            )
        val_data = load_preference_dataset(cast(Any, data_config["validation"]))
        val_task_data_processors = {
            val_data.task_name: (val_data.task_spec, preference_preprocessor)
        }
        if hasattr(val_data, "preprocessor") and val_data.preprocessor is not None:
