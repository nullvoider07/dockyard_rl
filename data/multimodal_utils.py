...
def get_multimodal_default_settings_from_processor(
    ...
    default_settings = {}
    _load_media_param_cache: dict[str, list[str]] = {}
    if hasattr(processor, "video_processor"):
        ...
        if "load_video_kwargs" not in _load_media_param_cache:
            _load_media_param_cache["load_video_kwargs"] = [
                param for param in inspect.signature(load_video).parameters
            ]
        default_settings["video"] = {
            arg: video_settings_dict[arg]
            for arg in _load_media_param_cache["load_video_kwargs"]
            if arg in video_settings_dict
        }
    if hasattr(processor, "feature_extractor"):
        if "load_audio_kwargs" not in _load_media_param_cache:
            _load_media_param_cache["load_audio_kwargs"] = [
        ...
        default_settings["audio"] = {
            ...
            for arg in _load_media_param_cache["load_audio_kwargs"]
        ...
        for vid in media_in_message["video"]:
            if isinstance(vid, str):
                ...
                loaded_media["video"].append(
                    load_video(cast(Any, vid), backend="decord", **load_video_kwargs)[0]
                )
            else:
