...
def _parallelize_qwen3_moe(
    model: Qwen3MoeForCausalLM,
    sequence_parallel: bool = False,
) -> dict[str, ParallelStyle]:
    """TP plan for a (surgically-converted) Qwen3-MoE model.

    Covers only attention + embeddings + lm_head. After ``apply_moe_surgery`` the
    decoder ``mlp`` is a native ``MoEBlock`` (router + grouped-GEMM experts), not
    the dense ``mlp.{gate,up,down}_proj``: its routed experts are sharded
    separately by the EP path (``parallelize_grouped_experts``) and the tiny
    router gate is replicated across TP, so neither belongs in this plan.

    Sequence parallelism is rejected: SP shards activations on the sequence dim
    and the dense plan stitches the boundary through ``mlp.down_proj``, which the
    ``MoEBlock`` does not expose. MoE-aware SP (token (un)shard around the
    dispatcher) is not built.
    """
    assert not sequence_parallel, (
        "sequence_parallel is not supported for Qwen3-MoE; the MoEBlock replaces "
        "the dense mlp the SP plan stitches through. Set dtensor_cfg.sequence_parallel=false."
    )
    base_model_tp_plan: dict[str, ParallelStyle] = {
        "lm_head": ColwiseParallel(output_layouts=Shard(-1), use_local_output=False),
        "model.embed_tokens": RowwiseParallel(input_layouts=Replicate()),
...
PARALLIZE_FUNCTIONS: dict[
    ...
    Qwen3MoeForCausalLM: _parallelize_qwen3_moe,
...
def get_hf_tp_plan(model: PreTrainedModel):
    ...
    if hasattr(inner_model, "_tp_plan") and inner_model._tp_plan is not None:
        hf_tp_plan.update(
            {f"{model_prefix}.{k}": v for k, v in cast(dict, inner_model._tp_plan).items()}
        )
...
def _parallelize_model(
    ...
    if model_cls == Gemma3ForConditionalGeneration:
        ...
        assert model.config.text_config is not None, "text_config must not be None"
        ...
        for layer in model.language_model.layers:  # type: ignore[union-attr]
            layers.append(layer)
        # append visual model layers
        for layer in model.visual.blocks:  # type: ignore[union-attr]
            layers.append(layer)
        ...
        layers: list = []  # type: ignore
        for layer in model.model.text_model.layers:  # type: ignore[union-attr]
            layers.append(layer)
        for layer in model.model.vision_model.encoder.layers:  # type: ignore[union-attr]
            layers.append(layer)
        ...
        layers: list = []  # type: ignore
        for layer in model.model.language_model.layers:  # type: ignore[union-attr]
            layers.append(layer)
        for layer in model.vision_tower.vision_model.encoder.layers:  # type: ignore[union-attr]
            layers.append(layer)
        num_attention_heads = model.language_model.config.num_attention_heads
        num_key_value_heads = model.language_model.config.num_key_value_heads
        ...
        for layer in model.model.language_model.layers:  # type: ignore[union-attr]
            layers.append(layer)
        for layer in model.model.vision_tower.transformer.layers:  # type: ignore[union-attr]
            layers.append(layer)
        ...
        layers: list = []  # type: ignore
        for layer in model.language_model.model.layers:  # type: ignore[union-attr]
            layers.append(layer)
        for layer in model.vision_model.model.layers:  # type: ignore[union-attr]
            layers.append(layer)
    ...
    if tp_mesh.size() > 1:
        assert isinstance(num_key_value_heads, int), f"num_key_value_heads must be an int, got {type(num_key_value_heads)}"
        assert isinstance(num_attention_heads, int), f"num_attention_heads must be an int, got {type(num_attention_heads)}"
