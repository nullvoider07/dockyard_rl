...
def my_init(*args, **kwargs):
    kwargs["vllm_config"].dockyard_fp8_cfg = global_fp8_config
    return original_init(*args, **kwargs)


def my_run_engine_core(*args, **kwargs):
    fp8_cfg = kwargs["vllm_config"].dockyard_fp8_cfg
    del kwargs["vllm_config"].dockyard_fp8_cfg
...
def monkey_patch_vllm_ray_executor(fp8_config):
    if fp8_config.model_parallel_size > 1:
        ...
        # vLLM v1 ships RayDistributedExecutor in `ray_executor` (the path used by
        # vllm.v1.executor.abstract); fall back to the legacy module name for
        # cross-version safety.
        try:
            from vllm.v1.executor.ray_executor import RayDistributedExecutor  # type: ignore[import]
        except ImportError:
            from vllm.v1.executor.ray_distributed_executor import RayDistributedExecutor  # type: ignore[import]
...
def is_fp8_model(vllm_config):
    ...
    if hasattr(vllm_config, "quant_config") and isinstance(
        ...
        assert vllm_config.quant_config.weight_block_size is not None, (
            "Only block scaling is currently supported in dockyard_rl!"
        )
        return True
...
def maybe_post_process_fp8_weight_block(layer: Any):
...
def process_weights_after_loading_moe(self, layer) -> None:
    ...
    """
    from vllm.model_executor.layers.quantization.fp8 import convert_to_fp8_moe_kernel_format  # type: ignore[reportPrivateImportUsage]
