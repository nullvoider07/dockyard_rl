...
from dockyard_rl.distributed.virtual_cluster import _get_free_port_local, _get_node_ip_local
...
from dockyard_rl.models.generation.sglang.utils import AsyncLoopThread

try:
    from dockyard_rl.utils.nsys import wrap_with_nvtx_name
except ImportError:  # nsys profiling helper not present — fall back to a no-op decorator

    def wrap_with_nvtx_name(name: str):  # type: ignore[misc]
        def decorator(fn):
            return fn
...
def _require_sglang():
    ...
    try:
        from sglang.srt.entrypoints.http_server import launch_server  # type: ignore[import]
        from sglang.srt.server_args import ServerArgs  # type: ignore[import]
        from sglang.srt.utils import kill_process_tree  # type: ignore[import]
    except ModuleNotFoundError as e:  # pragma: no cover
        raise ModuleNotFoundError(
            "Optional dependency `sglang` is required for the SGLang generation backend.\n"
            "Install the `sglang` package in the inference environment to use "
...
class SGLangGenerationWorker:
    ...
    def __init__(
        ...
        # Model owners always receive bundle_indices (is_model_owner is exactly
        # `bundle_indices is not None`, and we returned early above otherwise).
        ...
        # Build SGLang server arguments. Cast to a plain dict so the type checker
        # permits access to keys that are NotRequired on SglangSpecificArgs but are
        # required at runtime to launch the server (a missing key still raises
        # KeyError, which is the intended fail-loud behavior).
        sglang_cfg = cast(dict[str, Any], self.sglang_cfg)
        kwargs = {
            "model_path": sglang_cfg["model_path"],
            "trust_remote_code": True,
            "random_seed": seed
            if seed is not None
            else sglang_cfg.get("random_seed", 1),
            # Memory settings
            "enable_memory_saver": sglang_cfg["enable_memory_saver"],
            "gpu_id_step": 1,
            "base_gpu_id": base_gpu_id,
            # Parallel settings
            "tp_size": tp_size,
            "dp_size": sglang_cfg["dp_size"],
            "pp_size": sglang_cfg["pp_size"],
            "ep_size": sglang_cfg["ep_size"],
            # Always skip warmup to prevent warmup timeout
            "skip_server_warmup": sglang_cfg.get("skip_server_warmup", True),
        ...
        # Separate session bound to the Ray actor event loop for the async
        # generate_async path (the sync `generate` path runs on async_loop_thread,
        # whose loop differs; aiohttp sessions are not shareable across loops).
        self._actor_session: Optional[aiohttp.ClientSession] = None
    ...
    def _merge_stop_strings(self, batch_stop_strings):
        ...
        config_stop_strings = self.cfg.get("stop_strings")
        if config_stop_strings:
            stop_set.update(config_stop_strings)
    ...
    def _build_sampling_params(
        ...
        *,
        ...
        # Defensively cap max_new_tokens so input_len + max_new_tokens stays within
        # context_length. SGLang's SamplingParams does not clamp against context
        # (it only validates >= 0 / min_new_tokens), so without this the scheduler
        # would reject over-context requests instead of truncating them.
    ...
    async def _generate_single_sample(
        ...
        # stop must live inside sampling_params (a real SGLang SamplingParams field),
    ...
    async def _generate_async(
        ...
        return cast(list[tuple[list[int], list[float]]], results)
    ...
    @wrap_with_nvtx_name("sglang_generation_worker/generate")
    def generate(
        ...
        # _pad_token_id is NotRequired on the config type (populated by
        # configure_generation_config, not the caller) but always present at runtime.
        pad_token_id = cast(dict[str, Any], self.cfg)["_pad_token_id"]
    ...
    async def _ensure_actor_session(self) -> aiohttp.ClientSession:
        """aiohttp session bound to the Ray actor event loop (for generate_async)."""
        if self._actor_session is None or self._actor_session.closed:
            connector = aiohttp.TCPConnector(limit=512, limit_per_host=512)
            self._actor_session = aiohttp.ClientSession(
                connector=connector, timeout=aiohttp.ClientTimeout(total=300)
            )
        return self._actor_session

    async def generate_async(
        self, data: BatchedDataDict[GenerationDatumSpec], greedy: bool = False
    ) -> AsyncGenerator[tuple[int, BatchedDataDict[GenerationOutputSpec]], None]:
        """Stream a single sample's generation, yielding (sample_idx, result).

        Runs as a concurrent Ray async-actor method: the driver shards batches
        into per-sample calls and fires many in parallel, each issuing one
        SGLang HTTP request on the actor event loop. Mirrors the vLLM
        generate_async contract.
        """
        if not self.sglang_cfg.get("async_engine", False):
            raise RuntimeError(
                "generate_async requires sglang_cfg.async_engine=True."
            )
        ...
        batch_stop_strings = data.get("stop_strings", [None] * batch_size)
        stop_strings       = self._merge_stop_strings(batch_stop_strings)
        context_length     = self.sglang_cfg.get("context_length", None)
        ...
        if context_length is not None and input_len >= context_length:
            input_len = context_length - 1
        valid_input_ids = input_ids[0, :input_len].tolist()
        ...
        session = await self._ensure_actor_session()
        new_tokens, new_logprobs = await self._generate_single_sample(
            input_ids=valid_input_ids,
            sampling_params=sampling_params,
        ...
        total_len = input_len + n_gen

        full_output = torch.full((total_len,), pad_token_id, dtype=input_ids.dtype)
        full_output[:input_len] = input_ids[0, :input_len]
        if new_tokens:
            full_output[input_len:total_len] = torch.tensor(
                new_tokens, dtype=input_ids.dtype
            )
        ...
        result_batch = BatchedDataDict[GenerationOutputSpec](
            {
                ...
                "generation_lengths": torch.tensor([n_gen], dtype=torch.long),
                "unpadded_sequence_lengths": torch.tensor(
                    [total_len], dtype=torch.long
                ),
            }
        )
        yield (0, result_batch)

    def sleep(self, **kwargs) -> bool:
        """Release the server's GPU memory occupation (KV cache + weights).

        Posts to SGLang's /release_memory_occupation endpoint so the trainer can
        reclaim GPU memory between generation rounds. No-op for secondary
        (placeholder) workers, which never started a server.

        Args:
            tags: Optional list of memory tags to release. If omitted, the
                server releases all occupations.

        Returns:
            bool: True if the release succeeded (or this is not the model
            owner), False on failure.
        """
        if not self.is_model_owner:
            return True
        ...
        try:
            self._make_request("release_memory_occupation", payload)
            logger.info(
                f"[SGLang Worker] Rank {self.global_rank} Released memory occupation."
            )
            return True
        except Exception as e:
            logger.error(
                f"[SGLang Worker] Rank {self.global_rank} Failed to release memory occupation: {e}"
            )
            return False

    def wake_up(self, **kwargs) -> bool:
        """Reacquire the server's GPU memory occupation.

        Counterpart to sleep(): posts to SGLang's /resume_memory_occupation
        endpoint to restore GPU memory before the next generation round. No-op
        for secondary (placeholder) workers.

        Args:
            tags: Optional list of memory tags to resume. If omitted, the
                server resumes all occupations.

        Returns:
            bool: True if the resume succeeded (or this is not the model
            owner), False on failure.
        """
        if not self.is_model_owner:
            return True
        ...
        payload = {"tags": tags} if tags is not None else {}
        try:
            self._make_request("resume_memory_occupation", payload)
            logger.info(
                f"[SGLang Worker] Rank {self.global_rank} Resumed memory occupation."
            )
            return True
        except Exception as e:
            logger.error(
                f"[SGLang Worker] Rank {self.global_rank} Failed to resume memory occupation: {e}"
            )
            return False
        ...
        try:
            ...
            if hasattr(self, "session") and self.session is not None:
                try:
                    ...
                    # Capture into locals so the narrowing from the `is not None`
                    # guard above is preserved inside the nested coroutine.
