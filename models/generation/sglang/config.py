...
if TYPE_CHECKING:
    ...
    try:
        ...
        # interfaces.py not yet ported — degrade to a plain dict base so this
        # module stays importable (TypedDicts carry no runtime behaviour).
...
class SglangSpecificArgs(TypedDict):
    ...
    # When True, SGLangGeneration exposes the per-sample streaming
    # generate_async path (required for async GRPO), and the worker runs as a
    # concurrent async Ray actor. The SGLang HTTP server is always async; this
    # gates the driver-side streaming contract, mirroring vllm_cfg.async_engine.
