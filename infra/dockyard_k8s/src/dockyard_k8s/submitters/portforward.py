...
class PortForwardSubmitter:
    ...
    ``working_dir=None`` is supported — the Ray job inherits whatever cwd the
    head pod's default entrypoint ran with, which is useful when
    ``infra.launch.codeSource`` is ``image`` or ``lustre`` (code already on
    disk inside the container) but the user still wants Ray's Job API for
    tracking.

    Env vars are inlined as shell ``export``s in the entrypoint rather than
    sent through ``runtime_env.env_vars``. The latter conflicts with the
    captured ``os.environ`` that ``ray.init(address="auto")`` later hands to
    worker actors — Ray raises "Failed to merge the Job's runtime env" when it
    sees the same key in both. Keeping the transport's interface uniform with
    :class:`ExecSubmitter` (env via ``export``) avoids the whole class.
    """
    ...
    def stop(self, handle: SubmissionHandle, *, force: bool = False) -> None:
        # Ray's API has no force-kill vs graceful — stop_job asks the head to
        # SIGTERM the driver. Accept and ignore ``force``.
        del force
        from ray.job_submission import JobSubmissionClient
...
class _LazyRayStatusMap:
    ...
    Ray is a heavy import (~2 s of startup). The CLI has commands that never
    touch a dashboard, so the cost is only paid on first status lookup.
    """
...
def _prepend_env_exports(entrypoint: str, env_vars: dict[str, str]) -> str:
    ...
    Ray's submit_job entrypoint runs under /bin/dash by default, so bash-only
    syntax is avoided — each export lives on its own line with shell-quoted
    values. Invalid env keys raise immediately so a typo doesn't produce
    subtle failures deep in the job.
    """
    if not env_vars:
        return entrypoint
    lines = []
