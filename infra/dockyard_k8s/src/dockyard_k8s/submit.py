...
* **In-cluster**: when the CLI runs inside a pod that can reach the cluster
  DNS (``<raycluster>-head-svc.<ns>.svc.cluster.local:8265``), the dashboard
  is hit directly. Detected via ``KUBERNETES_SERVICE_HOST``.
...
Namespace, cluster name, and port are all passed in — no cluster-specific
assumptions.
"""
...
# Cache the kubectl probe for the process lifetime — auth doesn't flip
# between one command's checks.
...
def kubectl_preflight(namespace: str) -> None:
    ...
    if shutil.which("kubectl") is None:
        raise RuntimeError(
            "kubectl not found on PATH — install it or run dockyard-k8s from an "
            "in-cluster pod."
        )
    try:
        subprocess.run(
...
class _PortForward:
    ...
    ``restart()`` lets HTTP clients recover from a forward that died
    mid-call without tearing down the whole context.
    """
    ...
    def start(self) -> None:
        ...
        self._drain = threading.Thread(
            target=self._drain_pipe, name="dockyard-k8s-pf-drain", daemon=True
        )
        self._drain.start()
...
def tail_job_logs(dashboard: str, job_id: str) -> Iterator[str]:
    ...
    (including on KeyboardInterrupt) without coordinating event loops.
    """
    import asyncio
    import queue
    import threading
    ...
    threading.Thread(target=_worker, name="dockyard-k8s-tail", daemon=True).start()
    while True:
        item = q.get()
        if item is sentinel:
            return
...
def _wait_for_tcp(
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            time.sleep(0.5)
    raise TimeoutError(f"kubectl port-forward didn't open {host}:{port} in {timeout_s}s")
