...
RayCluster + RayJob are Kubernetes ``CustomObjectsApi`` resources, so the
client's generic CR helpers are used instead of modeling either CRD in
Python. The authored YAML stays authoritative; this module ships those
objects into the cluster and polls until they're ready. The sandbox pool is
a stock ``apps/v1`` Deployment + ``v1`` Service.
"""
...
# actively-running / suspended states where polling continues.
...
# The kubernetes client's methods are typed with very loose unions (the
# async_req overloads return AsyncResult; the sync path returns an untyped
# deserialized blob), so the API handles are surfaced as ``Any`` — accessing
# ``.status`` / ``.spec`` / ``.items`` on a typed handle would otherwise trip
# a cascade of false-positive attribute errors.
def custom_objects_api() -> Any:
    load_kubeconfig()
...
def apply_raycluster(manifest: dict[str, Any], namespace: str) -> dict[str, Any]:
    ...
    try:
        ...
        # Attach a redacted manifest summary for debugging without leaking
        # secret env values into the CLI output.
        exc.dockyard_k8s_manifest = redact(manifest)  # type: ignore[attr-defined]
        raise


def delete_raycluster(name: str, namespace: str, *, ignore_missing: bool = True) -> None:
    api = custom_objects_api()
    try:
        with_retries(
...
def wait_for_rayjob_terminal(
    ...
    *,
    timeout_s: int = 86400,
    poll_s: int = 10,
    on_update: Callable[[str | None, str | None], None] | None = None,
) -> dict[str, Any]:
...
#
# Both are CustomObjectsApi resources. apply is a no-op on 409 (the CR already
# exists from a prior run); delete tolerates 404.
...
def delete_compute_domain(name: str, namespace: str, *, ignore_missing: bool = True) -> None:
    api = custom_objects_api()
    try:
        with_retries(
            lambda: api.delete_namespaced_custom_object(
...
def apply_resource_claim_template(manifest: dict[str, Any], namespace: str) -> dict[str, Any]:
    api = custom_objects_api()
    try:
        return with_retries(
            lambda: api.create_namespaced_custom_object(
...
def get_head_pod(cluster_name: str, namespace: str) -> Any:
    ...
    head pod with ``ray.io/cluster=<name>,ray.io/node-type=head``.
    """
    core = _core_api()
        ...
        if exc.status == 404:
            secret = client.V1Secret(
                metadata=client.V1ObjectMeta(
                    name=name,
                    namespace=namespace,
                    labels={"app.kubernetes.io/managed-by": "dockyard-k8s"},
                ),
                type="Opaque",
                data=encoded,
            )
...
def apply_deployment(manifest: dict[str, Any], namespace: str) -> Any:
    ...
    try:
        return with_retries(
            lambda: apps.create_namespaced_deployment(namespace=namespace, body=manifest)
        )
    except ApiException as exc:
        if exc.status == 409:
            return with_retries(
...
def delete_deployment(name: str, namespace: str, *, ignore_missing: bool = True) -> None:
    apps = _apps_api()
    try:
        with_retries(
...
# NetworkPolicy lifecycle (networking.k8s.io/v1)

def _networking_api() -> Any:
    load_kubeconfig()
    return client.NetworkingV1Api()


def apply_network_policy(manifest: dict[str, Any], namespace: str) -> Any:
    """Create-or-patch a NetworkPolicy. Returns the server-side object."""
    name = manifest["metadata"]["name"]
    net = _networking_api()
    try:
        return with_retries(
            lambda: net.create_namespaced_network_policy(namespace=namespace, body=manifest)
        )
    except ApiException as exc:
        if exc.status == 409:
            return with_retries(
                lambda: net.patch_namespaced_network_policy(
                    name=name, namespace=namespace, body=manifest
                )
            )
        raise
...
def delete_network_policy(name: str, namespace: str, *, ignore_missing: bool = True) -> None:
    net = _networking_api()
    try:
        with_retries(
            lambda: net.delete_namespaced_network_policy(name=name, namespace=namespace)
        )
    except ApiException as exc:
        if exc.status == 404 and ignore_missing:
            return
...
# PodDisruptionBudget lifecycle (policy/v1)
...
def apply_pod_disruption_budget(manifest: dict[str, Any], namespace: str) -> Any:
    """Create-or-patch a PodDisruptionBudget. Returns the server-side object.

    The PDB ``selector`` is immutable; for our objects it is name-derived and
    stable, so a patch on 409 only changes min/maxUnavailable.
    """
    name = manifest["metadata"]["name"]
    pol = _policy_api()
    try:
        return with_retries(
            lambda: pol.create_namespaced_pod_disruption_budget(
        ...
        if exc.status == 409:
            return with_retries(
                lambda: pol.patch_namespaced_pod_disruption_budget(
                    name=name, namespace=namespace, body=manifest
                )
            )
        raise
...
def delete_pod_disruption_budget(
    name: str, namespace: str, *, ignore_missing: bool = True
) -> None:
    pol = _policy_api()
    try:
        with_retries(
            lambda: pol.delete_namespaced_pod_disruption_budget(
