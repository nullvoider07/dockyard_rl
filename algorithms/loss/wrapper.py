...
from dockyard_rl.algorithms.loss.loss_functions import DraftCrossEntropyLossFn
...
class SequencePackingLossWrapper:
    def __init__(
        ...
        Args:
            ...
            vocab_parallel_rank, vocab_parallel_group, context_parallel_group are only used by vocab/context-parallel workers.
    ...
    def __call__(
        ...
        loss_accum: torch.Tensor = torch.zeros(
        ...
        for seq_idx in range(len(seq_starts)):
            seq_start = int(seq_starts[seq_idx].item())
            seq_end   = int(seq_ends[seq_idx].item())
            ...
            if (
                ...
                logit_start  = seq_start // cp_size
                logit_end    = (seq_start + int(padded_seq_lengths[seq_idx].item())) // cp_size
            ...
            loss, metrics = self.loss_fn(
                data=unpadded_seq_data,
                global_valid_seqs=global_valid_seqs,  # type: ignore[arg-type]
                global_valid_toks=global_valid_toks,  # type: ignore[arg-type]
                **loss_input,
            )
...
class SequencePackingFusionLossWrapper:
    ...
    The prepare_fn should be prepare_packed_loss_input (from dockyard_rl.algorithms.loss.utils),
...
def wrap_loss_fn_with_input_preparation(
    ...
    """Wraps a loss function to handle input preparation for the policy worker."""
    # prepare loss input
